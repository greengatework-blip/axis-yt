...
def main():
    ...
    # ffmpeg filtergraph (文字は時間で切替)
    vf = (
        "scale=1080:1920,format=yuv420p,"
        # Header bar
        "drawbox=x=0:y=0:w=1080:h=160:color=white@0.08:t=fill,"
        "drawtext=text='AXIS • DEEP SPACE':fontcolor=white:fontsize=36:"
        "x=(w-text_w)/2:y=60,"
        # 3 blocks
        f"drawtext=text='{HOOK}':fontcolor=white:fontsize=60:"
        "x=(w-text_w)/2:y=420:enable='between(t,0,6)',"
        f"drawtext=text='{QUESTION}':fontcolor=white:fontsize=54:"
        "x=(w-text_w)/2:y=420:enable='between(t,6,12)',"
        f"drawtext=text='{ANSWER}':fontcolor=white:fontsize=46:"
        "x=(w-text_w)/2:y=420:enable='between(t,12,18)',"
        # Bottom CTA
        "drawbox=x=0:y=1680:w=1080:h=240:color=white@0.06:t=fill,"
        "drawtext=text='Follow for daily space insights':fontcolor=white:fontsize=34:"
        "x=(w-text_w)/2:y=1760"
    )
